...
import { FeatureNavCard } from '@/features/workspaces/components/FeatureNavCard';
...
export function WorkspaceDetailPage() {
  ...
  const { data: calendarTasks } = useCalendarTasks(workspaceId, monthKey);
  const { data: analytics } = useAnalytics(workspaceId);
  const totalTasks = analytics?.statusBreakdown.reduce((sum, s) => sum + s.count, 0);
    ...
    <div className="mx-auto max-w-3xl px-4 py-8">
      ...
      <div className="mb-10 grid grid-cols-1 gap-4 sm:grid-cols-2">
        <FeatureNavCard
          ...
          metric={calendarTasks ? `${calendarTasks.length} due this month` : undefined}
        />
        <FeatureNavCard
          to={`/workspaces/${workspaceId}/analytics`}
          icon={ChartIcon}
          title="Analytics"
          description="Track workspace progress"
          metric={totalTasks !== undefined ? `${totalTasks} tasks tracked` : undefined}
        />
      </div>

      <div className="divide-y divide-border">
        <section className="pb-10">
          <h2 className="mb-4 text-lg font-medium text-text">Search tasks</h2>
          <SearchPanel workspaceId={workspaceId} />
        </section>

        <section className="py-10">
          <h2 className="mb-4 text-lg font-medium text-text">Boards</h2>{' '}
          {canManageBoards && (
        ...
        <section className="py-10">
          <h2 className="mb-4 text-lg font-medium text-text">Members</h2>
        ...
        <section className="pt-10">
          <h2 className="mb-4 text-lg font-medium text-text">Activity</h2>
